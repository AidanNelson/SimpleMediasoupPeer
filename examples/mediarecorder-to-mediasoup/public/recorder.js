...
function handleDataAvailable(event) {
  ...
  if (event.data && event.data.size > 0) {
    ...
    socket.emit("media-chunk", { chunk: event.data, uuid: recordingUUID });
  }
}
...
async function startRecording() {
  ...
  mediaRecorder.onstop = (event) => {
    ...
    socket.emit('recording-end', {uuid: recordingUUID});
  };
  mediaRecorder.ondataavailable = handleDataAvailable;
  mediaRecorder.start(1000);
  recordingUUID = self.crypto.randomUUID();
  console.log("Starting recording: ", recordingUUID);
  socket.emit("recording-start", { uuid: recordingUUID });
